...
def _build_sing_notes(
...
) -> Optional[tuple[int, list[tuple[float, float, int, str]]]]:
    """
    从篇章构建歌唱用音符列表（不合并连音，保证音节与音符一一对应）。
    返回 (voice_id, [(start_time, duration, midi, lyric), ...]) 或 None（无 voice_id）
    """
    ...
    if section_index < len(section_settings):
        s = section_settings[section_index]
    else:
        s = score.settings
    bpm = s.bpm
...
def _notes_to_voicevox_format(
    notes: list[tuple[float, float, int, str]],
) -> list[dict]:
    """将 (start_time, duration, midi, lyric) 转为 VOICEVOX /sing_frame_audio_query 的 notes 格式。hold 已合并到前音，此处不再出现。首个必须是空 note。"""
    out: list[dict] = [
        {"id": str(uuid.uuid4()), "key": None, "frame_length": 2, "lyric": ""},
    ]
    for start_time, duration, midi, lyric in notes:
        if _is_hold_lyric(lyric):
            continue  # 理论上已合并，防御性跳过
        ...
        out.append({
            ...
            "lyric": lyric.strip() if lyric else "あ",  # 空则用默认
        })
    return out
        ...
        if seg.notes:
            ...
            if can_sing:
                sing_result = _synthesize_section(
                    ...
                    max_duration_seconds=seg_dur,
                    voice_id_override=voice_id_override,
                    base_url=base_url,
                )
                if sing_result:
                    seg_audio, _ = sing_result
                    target_len = int(seg_dur * sample_rate) + 1
