...
def show_voicevox_model_dialog(parent: tk.Tk) -> None:
    ...
    def _refresh_status() -> None:
        ...
        try:
            ...
            if get_voicevox_backend() == "docker":
                lines.append("提示：当前设置为 Docker 模式，音色来自 voicevox_engine 容器。此功能用于本地库模式。")
        except Exception:
            pass
        if is_voicevox_core_installed():
            lines.append("✓ voicevox_core 已安装")
        else:
            url = get_wheel_url()
            lines.append("✗ voicevox_core 未安装" + (f"（可安装，平台支持）" if url else "（当前平台暂无预编译包）"))

        dict_dir = get_open_jtalk_dict_dir()
        if dict_dir.exists():
            lines.append("✓ Open JTalk 辞書已就绪")
        else:
            lines.append("✗ Open JTalk 辞書未安装（需运行「安装依赖」）")

        installed = [e.filename for e in VVM_CATALOG if is_vvm_installed(e.filename)]
        lines.append(f"已下载模型: {', '.join(installed) if installed else '无'}")
        if is_core_ready():
            lines.append("\n✓ 本地模式可用，无需 Docker")
        else:
    ...
    ttk.Button(btn_frame, text="安装 voicevox_core", command=lambda: _do_install(_status)).pack(side=tk.LEFT, padx=(0, 5))
    ttk.Button(btn_frame, text="安装 ONNX/辞書", command=lambda: _do_deps(_status)).pack(side=tk.LEFT, padx=(0, 5))
    ttk.Button(btn_frame, text="刷新状态", command=_refresh_status).pack(side=tk.LEFT, padx=(0, 5))

    # 模型列表
    list_frame = ttk.LabelFrame(main, text="可选音声模型（选择后点击下载/删除，Ctrl+点击多选）", padding=8)
    ...
    tree.heading("installed", text="已下载")
    ...
    def _do_download() -> None:
        ...
        if not selected:
            messagebox.showinfo("提示", "请先选择要下载的模型（Ctrl+点击可多选）")
            return
        def _run():
            for f in selected:
                ok, err = download_vvm(f, status_callback=_status)
                if not ok:
                    _status(f"下载 {f} 失败: {err}")
                else:
                    _status(f"✓ {f} 下载完成")
    ...
    ttk.Button(btn_frame, text="下载选中模型", command=_do_download).pack(side=tk.LEFT, padx=(20, 0))

    def _do_delete() -> None:
        selected = list(tree.selection())
        if not selected:
            messagebox.showinfo("提示", "请先选择要删除的模型（仅可删除已下载的，Ctrl+点击可多选）")
            return
        to_delete = [f for f in selected if is_vvm_installed(f)]
        if not to_delete:
            messagebox.showinfo("提示", "所选模型未下载，无需删除")
            return
        names = "、".join(to_delete)
        if not messagebox.askyesno("确认删除", f"确定要删除以下模型吗？\n{names}\n\n删除后可重新下载。"):
            return
        for f in to_delete:
            ok, err = delete_vvm(f)
            if not ok:
                messagebox.showerror("删除失败", f"{f}: {err}")
            else:
                _status(f"✓ 已删除 {f}")
                tree.set(f, "installed", "")
        _refresh_status()

    ttk.Button(btn_frame, text="删除选中模型", command=_do_delete).pack(side=tk.LEFT, padx=(5, 0))

    def _do_install(cb) -> None:
        def _run():
            ok, err = install_voicevox_core(status_callback=cb)
            if ok:
                cb("✓ voicevox_core 安装成功")
            else:
                cb(f"✗ 安装失败: {err}")
            _safe_ui(dlg, _refresh_status)
        threading.Thread(target=_run, daemon=True).start()
    ...
    def _do_deps(cb) -> None:
        def _run():
            cb("正在下载并运行 voicevox_core 官方安装脚本...")
            ok, err = run_download_script(status_callback=cb)
            if ok:
                cb("✓ ONNX Runtime 与 Open JTalk 辞書安装完成")
            else:
                cb(f"✗ 安装失败: {err}")
            _safe_ui(dlg, _refresh_status)
        threading.Thread(target=_run, daemon=True).start()

    ttk.Button(main, text="关闭", command=dlg.destroy).pack(anchor=tk.E)
    _refresh_status()
...
def show_vvm_download_dialog(parent: tk.Tk, filename: str) -> tuple[bool, str]:
    ...
    ttk.Label(main, text=f"正在下载 {filename}...").pack(anchor=tk.W)
    ...
    threading.Thread(target=_run, daemon=True).start()
    dlg.wait_window()
    return result[0] if result else (False, "未完成")
...
def show_style_not_found_dialog(
    ...
    if not messagebox.askyesno("需要下载音声模型", f"使用此音色需要下载 {vvm}。\n是否立即下载？"):
        return False
    ok, err = show_vvm_download_dialog(parent, vvm)
    if ok:
        messagebox.showinfo("下载完成", f"{vvm} 已下载，请重试。")
        return True
    if err:
